...
Adds mutating maintenance actions, selected-plan quota adapters, and explicit
Apocalypse-owned analysis refreshes while preserving the existing UI contracts.
"""
...
# spatial.ops() resolves its module-global `quotas` function at runtime.
...
def repair_conversation(session_id: str):
    """Remove invalid JSONL records from one stopped Claude transcript, safely."""
    ...
    for idx, raw in enumerate(raw_lines, start=1):
        ...
        nonempty += 1
        try:
            json.loads(raw.decode("utf-8")); good.append(raw)
        except Exception as exc:
            bad.append({"line": idx, "error": str(exc)[:160]})
    if not bad:
        return {"ok": True, "changed": False, "session_id": session_id,
                "total_records": nonempty, "valid_records": len(good), "removed_records": 0,
                "message": "Conversation file is already valid."}, None, 200
    try:
        now = tpath.stat()
    ...
    return {"ok": True, "changed": True, "session_id": session_id,
            "total_records": nonempty, "valid_records": len(good), "removed_records": len(bad),
            "removed": bad[:50], "backup": str(backup), "transcript": str(tpath)}, None, 200
...
def _cached_analysis():
    try:
        ...
        return {"generated_at": None, "schedule": None, "worklog": None}
...
class Handler(spatial.Handler):
    def do_GET(self):
        ...
        if path == "/api/analysis":
            return self.send_json(_cached_analysis())
        return super().do_GET()
    ...
    def do_POST(self):
        ...
        if path == "/api/analysis/refresh":
            try:
                return self.send_json({"ok": True, **ops_analysis.refresh(schedule=True, worklog=True)})
            except Exception as exc:
